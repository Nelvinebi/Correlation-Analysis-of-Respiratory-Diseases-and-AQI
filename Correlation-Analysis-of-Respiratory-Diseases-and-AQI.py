...
def generate_synthetic_data(n=500, random_state=42):
    ...
    population = rng.integers(5000, 200000, size=n)
    pop_density = rng.uniform(100, 10000, size=n)
    median_age = rng.normal(30, 8, size=n).clip(5, 90)
    smoking_rate = rng.uniform(0.05, 0.35, size=n)
    healthcare_index = rng.uniform(0.2, 1.0, size=n)
    temperature_C = rng.normal(25, 6, size=n)
    humidity = np.clip(rng.normal(0.6, 0.15, size=n), 0.1, 0.99)
    pm25 = np.abs(rng.normal(30, 20, size=n))
    pm10 = np.abs(rng.normal(50, 30, size=n))
    no2 = np.abs(rng.normal(25, 15, size=n))
    so2 = np.abs(rng.normal(5, 3, size=n))
    aqi = (0.6 * (pm25 / (pm25.max()+1) * 200) +
           0.3 * (pm10 / (pm10.max()+1) * 150) +
           0.1 * (no2 / (no2.max()+1) * 100))
    aqi = aqi + rng.normal(scale=5.0, size=n)
    ...
    linear_term = (intercept +
                   ...
                   beta_humid * humidity)
    expected_cases = np.exp(linear_term) * (rng.uniform(0.8, 1.2, size=n))
